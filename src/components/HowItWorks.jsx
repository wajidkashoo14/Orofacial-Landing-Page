...
const steps = [
    ...
    mockup: (
      <div className="bg-navy rounded-xl overflow-hidden shadow-xl">
        <div className="bg-navy-950 px-4 py-2.5 flex items-center justify-between">
          ...
          <span className="text-[10px] text-green-400 border border-green-400/30 px-2 py-0.5 rounded-full">⊕ MFA Active</span>
        </div>
        <div className="p-4">
          ...
          {['Sarah Mitchell', 'James Rivera', 'Anna Chen'].map((name, i) => (
            <div key={name} className="flex items-center gap-2 py-2 border-b border-white/5">
              <span className={`w-2 h-2 rounded-full ${i === 0 ? 'bg-red-400' : i === 1 ? 'bg-yellow-400' : 'bg-green-400'}`} />
              <span className="text-white text-xs">{name}</span>
            </div>
          ))}
          <div className="mt-3">
            <div className="text-white/40 text-[10px] tracking-widest uppercase mb-2">Upload & Assign Exercise</div>
            <div className="bg-navy-800 rounded-lg p-3 flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-gold/20 flex items-center justify-center">
                <Video size={14} className="text-gold" />
              </div>
              <div>
                <div className="text-white text-xs font-medium">Jaw Opening Exercise</div>
  ...
  },
  {
    ...
    mockup: (
      <div className="bg-white rounded-xl border border-navy/10 shadow-xl p-5 max-w-[220px] mx-auto">
        ...
        {[
          ...
          <div key={ex.name} className="flex items-start gap-3 py-2.5 border-b border-navy/6 last:border-0">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${ex.done ? 'bg-gold/20' : 'bg-navy/8'}`}>
              {ex.done ? <Check size={12} className="text-gold" /> : <Video size={12} className="text-navy/40" />}
            </div>
            <div>
      ...
      <div className="bg-navy rounded-xl overflow-hidden shadow-xl">
        ...
        <div className="p-4 space-y-2">
          {[
            ...
            <div key={p.name} className="flex items-center justify-between bg-navy-800/60 rounded-lg px-3 py-2">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full bg-white/10 flex items-center justify-center text-white text-[10px] font-medium">
                  {p.name[0]}
                </div>
                <span className="text-white text-xs">{p.name}</span>
              </div>
              <span className={`text-[9px] font-semibold px-2 py-0.5 rounded ${p.color}`}>{p.status}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <section id="how-it-works" className="py-24 bg-white" ref={ref}>
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className={`text-center mb-14 reveal ${visible ? 'visible' : ''}`}>
          ...
          <h2 className="font-serif text-4xl md:text-5xl font-medium text-navy mt-3">How it works</h2>
          <p className="text-navy/55 text-sm mt-4">Three steps. Two sides. One continuous treatment relationship.</p>
        </div>

        {/* Step tabs */}
        <div className={`flex flex-wrap justify-center gap-1 bg-navy/5 rounded-xl p-1.5 mb-12 max-w-xl mx-auto reveal delay-100 ${visible ? 'visible' : ''}`}>
          {steps.map((s, i) => (
            <button
              key={s.tab}
            ...
            >
              <span className={`w-5 h-5 rounded-full text-xs flex items-center justify-center font-semibold ${activeStep === i ? 'bg-white/20' : 'bg-navy/10'}`}>
                {s.number}
              </span>
              {s.tab}
            </button>
        ...
        {/* Step content */}
        <div key={activeStep} className="grid lg:grid-cols-2 gap-12 items-center">
          {/* Left: text */}
          <div className="space-y-6">
            <div>
              <div className="section-label mb-2">{step.role}</div>
              <h3 className="font-serif text-3xl md:text-4xl font-medium text-navy leading-snug">
                {step.title}
              </h3>
            </div>
            ...
            <div className="flex items-center gap-4 pt-2">
              ...
              >
                Next: {steps[(activeStep + 1) % steps.length].tab} <ArrowRight size={14} />
              </button>
              <div className="flex gap-1.5">
                {steps.map((_, i) => (
                  <button
                    key={i}
                    onClick={() => setActiveStep(i)}
                    className={`w-2 h-2 rounded-full transition-all ${i === activeStep ? 'bg-navy w-6' : 'bg-navy/20'}`}
                  />
                ))}
              </div>
            </div>
